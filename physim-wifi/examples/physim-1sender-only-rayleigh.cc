...
public:
  void
  Run (std::string wifiMode, uint32_t seed, double txpower, uint32_t packetSize, double simulationTime, double pathLoss, double bgNoise, double speed)
  {
    ...
    Config::SetDefault ("ns3::PhySimRicianPropagationLoss::MinimumRelativeSpeed", DoubleValue (speed) );
    Config::SetDefault ("ns3::PhySimRicianPropagationLoss::UseShortcut", BooleanValue (false) );
  ...
  void Save (std::string filename, uint32_t seed, uint32_t size, double speed)
  {

    std::stringstream sOut, pOut, spOut;
    sOut << seed;
    pOut << size;
    spOut << speed;
    filename.append ("-PayloadSize-");
    filename.append (pOut.str ());
    filename.append ("-RelativeSpeed-");
    filename.append (spOut.str ());
...
main (int argc, const char *argv[])
{
  ...
  if (argc < 9)
    {
      std::cout << "Usage: " << argv[0] << " --mode <wifiMode> --payload <frameSize> --seed <no> --speed <relativeSpeed>" << std::endl;
  ...
  uint32_t seed = 1;
  double relativeSpeed = 10;            // = 10 meters/sec
  ...
  std::cout << " - Relative Speed     = " << relativeSpeed << " meters/sec" << std::endl;
  ...
  for (double d = minPathLoss; d <= maxPathLoss; d += stepSize)
    {
      ...
      e.Run (wifiMode, seed, txPower, payloadSize, simulationTime, d, backgroundNoise, relativeSpeed);
    }

  e.Save (wifiMode, seed, payloadSize, relativeSpeed);
