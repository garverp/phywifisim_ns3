...
PhySimWifiTransmitterReceiverTest::DoRun (void)
{
  ...
  std::string modes[] = { "OfdmRate6Mbps","OfdmRate9Mbps","OfdmRate12Mbps","OfdmRate18Mbps","OfdmRate24Mbps","OfdmRate36Mbps","OfdmRate48Mbps","OfdmRate54Mbps"};
  for (int i = 0; i < 8; ++i)
    {
      RunSingle (modes[i]);
      if (!m_success)
      ...
      NS_TEST_EXPECT_MSG_EQ ( m_success, true, "Transmitter Receiver test failed for " << modes[i] << " : frame was not successfully decoded under ideal channel conditions");
    }
}
